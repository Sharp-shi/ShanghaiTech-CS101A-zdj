...
Book::Book(const Book& other)
    : bookId(other.bookId), relatedBookCount(other.relatedBookCount),isbn(other.isbn), 
    title(other.title), author(other.author), genre(other.genre),
    publicationYear(other.publicationYear), isAvailable(other.isAvailable),
    borrowedBy(other.borrowedBy), borrowDate(other.borrowDate), dueDate(other.dueDate) {
    if (other.relatedBookCount>0 && other.relatedBookId!=nullptr) {
        relatedBookId=new int[relatedBookCount];
        std::copy(other.relatedBookId,other.relatedBookId+relatedBookCount,relatedBookId);
    } else {
...
Book::Book(Book&& other) noexcept
    : bookId(other.bookId), relatedBookCount(other.relatedBookCount),relatedBookId(other.relatedBookId),
    isbn(std::move(other.isbn)), title(std::move(other.title)), author(std::move(other.author)), 
    genre(std::move(other.genre)),publicationYear(other.publicationYear), isAvailable(other.isAvailable),
    borrowedBy(std::move(other.borrowedBy)), borrowDate(other.borrowDate), dueDate(other.dueDate){
...
void Book::setRelatedBook(std::size_t count,const int* relatedBooks){
    ...
    if (count>0 && relatedBooks!=nullptr) {
        relatedBookId=new int[count];
        std::copy(relatedBooks,relatedBooks+count,relatedBookId);
    } else {
        relatedBookId=nullptr;
        relatedBookCount=0;
    }
